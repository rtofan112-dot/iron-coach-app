...
[System.IO.File]::WriteAllText("$dir\bundle.html", $bundle, [System.Text.Encoding]::UTF8)
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/ai-coach" && request.method === "POST") {
      try {
        const { prompt, athleteProfile, conversationHistory } = await request.json();
...
5. <b>Цинк хелат (25 мг):</b> Вечером после еды.`;
  ...
  // Universal intelligent dynamic response
...
$workerCode = $template.Replace('__B64_APP_PLACEHOLDER__', $b64)
[System.IO.File]::WriteAllText("$dir\worker.js", $workerCode, [System.Text.Encoding]::UTF8)
Write-Host "Bundle & worker.js created! Worker size: " $workerCode.Length
