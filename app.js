...
const DEFAULT_PROGRAMS = {
  ...
  b: {
    ...
    exercises: [
      ...
      { name: "Отжимания на брусьях (с акцентом на грудь)", muscleGroup: "Грудь", targetMuscles: "Низ груди • Трицепс", phases: ["01: Наклон 30°", "02: Угол 90°", "03: Выжим"], sets: 4, min: 8, max: 10, w: 0, calRate: 11, tip: "Корпус прямо, акцент на трицепс.", substitutes: ["Жим гантелей на горизонтальной скамье"] },
...
function renderInteractiveAnatomyMap() {
    ...
    const callout = `<g class="anat-selection-callout" aria-hidden="true" style="pointer-events:none;">
      ...
      <circle cx="${point[0]}" cy="${point[1]}" r="12" fill="transparent" stroke="#dfc299" stroke-width="1" opacity="0.5"/>
      <text x="280" y="${point[1] - 6}" text-anchor="end" fill="#ffffff" font-size="9" font-family="var(--font-sans)" font-weight="bold">${labels[selKey] || 'MUSCLE'}</text>
    </g>`;
    ...
    const svg = `<svg class="anatomy-atlas-svg" width="100%" height="100%" viewBox="0 0 320 500" fill="none" xmlns="http://www.w3.org/2000/svg" role="img">
      ${anatomyBase(viewLabel, currentAnatomyView === 'back')}
      ${regions}
      ${callout}
    </svg>`;
